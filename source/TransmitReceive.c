...
void Board_Init()
{
	...
	NVIC_SetPriority ( 79, 14 );
	NVIC_SetPriority ( 78, 16 );
...
void Transmit(flexcan_frame_t *txFrame)
{
	int retval;
...
Receive(flexcan_frame_t *RecvFrame)
{
	char CanDataIn[9];
	uint32_t MESSAGE_MASK =  1<<RX_MESSAGE_BUFFER_INDEX | 1<<RX_RTR_REQUEST ;
	...
	{ //loop to check for messages...
		...
		if ( RxFlags & (1<<RX_MESSAGE_BUFFER_INDEX) )
		{
			...
			FLEXCAN_ReadRxMb(CAN0, RX_MESSAGE_BUFFER_INDEX, RecvFrame);
			...
			printf("\n\rCleaning buffer.");
			printf("\n\rFrame length: < %d >",RecvFrame->length);

			ReadCanData(RecvFrame,CanDataIn);
			return(CanDataIn);
		...
		if ( RxFlags & (1<<RX_RTR_REQUEST) )
		{
			...
			//Reads the received message from the receive message buffer.
			FLEXCAN_ReadRxMb(CAN0, RX_RTR_REQUEST, RecvFrame);
			...
			return(ReadCanData(RecvFrame,CanDataIn));
			//printf("\n\rCanDataIn: ->%s<-\n\r",CanDataIn);
...
ReceiveCANMesg(flexcan_frame_t *RecvFrame)
{
	char CanDataIn[9];
	uint32_t MESSAGE_MASK =  1<<RX_MESSAGE_BUFFER_INDEX | 1<<RX_RTR_REQUEST ;
...
void Transmit_RTR(flexcan_frame_t *txFrame)
{
	int retval;
	FLEXCAN_SetTxMbConfig(CAN0, TX_MESSAGE_BUFFER_INDEX, true);
